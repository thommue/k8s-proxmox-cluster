...
from .._setupUtils import get_pwd, kubeadm_init, setup_calico, SSHConnectionPool, execute_command
...
class ClusterSetup:
    ...
    def setup_cluster(
        ...
        temp_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "templates", cluster_type.name.lower())
        ...
        with sftp.open(f"{pwd}/kubeadm-config.yaml", "w") as remote_file:
            remote_file.write(
                cls._setup_kubeadm_conf(
                    ...
                    master_names=[master.vm_name for master in group_vms[VmType.MASTER.name]],
                    master_ips=[master.ip_address for master in group_vms[VmType.MASTER.name]],
        ...
        kubeadm_master, kubeadm_worker = kubeadm_init(
            client=client_master, logger=logger, complex_type=True if cluster_type == ClusterType.COMPLEX else False
        )
        ...
        # join the master
        if cluster_type == ClusterType.COMPLEX:
            # move the certs
            cls._distribute_kube_certs(
                vms=[vm for vm in group_vms[VmType.MASTER.name] if vm.ip_address != master_vm.ip_address],
    ...
    def _exc_kubeadm_cmd(
        ...
        for vm in vms:
            ...
            logger.info(f"Connect to {vm.vm_name} {vm.ip_address}: {stdout.read().decode()} | {stderr.read().decode()}")
            sleep(15)
